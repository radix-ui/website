...
- `Slider.unstable_ThumbProvider` provides the thumb state and accepts an optional `name` for the submitted value.
- `Slider.unstable_ThumbTrigger` is the draggable thumb element.
- `Slider.unstable_BubbleInput` is the visually hidden input that `Slider.Thumb` renders by default. Omit it when you don't need form submission.
