	...
	data={[
		{
			...
			description: (
				<span>
					...
					part, it will be inherited by <Code>Select.Content</Code>.
				</span>
			),
		},
		{
...
By default, `Select.Root` renders a visually hidden `select` for form submission. To recompose, move, or exclude that element, you can build the select from its lower-level parts instead.

> **Important:** These parts are unstable and prefixed with `unstable_`, so their API may change in a future release.

- `Select.unstable_Provider` provides the select state and accepts the form-related and state props (`name`, `value`, `defaultValue`, `onValueChange`, etc.).
- `Select.unstable_BubbleInput` is the visually hidden `select` that `Select.Root` renders by default. Omit it when you don't need form submission.
