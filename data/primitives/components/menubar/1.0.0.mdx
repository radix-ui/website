...
An item that opens a submenu. Must be rendered inside `Menubar.SubMenu`.
...
The component that pops out when a submenu is open. Must be rendered inside `Menubar.SubMenu`.
...
You can create submenus by using `Menubar.SubMenu` in combination with its parts.
