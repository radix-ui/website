...
### Asynchronous form submissions within Dialog
...
export default () => {
  const [isFormDialogOpen, setFormDialogOpenState] = useState(false);

  return (
    <Dialog.Root onOpenChange={setFormDialogOpenState} open={isFormDialogOpen}>
      <Dialog.Trigger />
      <Dialog.Portal>
        <Dialog.Overlay>
          <Dialog.Content>
            <form
              onSubmit={async () => {
                somePromise().then(() => setFormDialogOpenState(false));
              }}
            >
              {/** some inputs */}
