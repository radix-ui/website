...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: (
        <>
          Use this prop to indicate on which condition the message should be
          visible.
        </>
      ),
    },
...
> Note: At then moment, it is not possible to compose `Form` with Radix's other form primitives such as `Checkbox`, `Select`, etc. We are working on a solution for this.
