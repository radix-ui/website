...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: (
        <span>
          The positioning mode to use. <Code>item-aligned</Code> will use some
          heuristics based on the items to position the content over the trigger
          trying to align it with the selected option at best (similar to what
          MacOS does). <Code>popper</Code> uses the same engine as all other
          components like <Code>Popover</Code> or <Code>DropdownMenu</Code> for
          example.
        </span>
      ),
    },
