...
If you need more flexibility, you can controlling the component using `value`/`onValueChange` props and passing `children` to `SelectValue`. Remember to make sure what you put in there is accessible.
...
```jsx line=5-8,10-13,15,17-20,27-28,32-36
import { styled } from '@stitches/react';
import * as Select from '@radix-ui/react-select';
...
const ScrollAreaScrollbar = styled(ScrollAreaPrimitive.Scrollbar, { width: 4 });
