...
<Box mt="3" mb="6">
  [View introduction blog post](/blog/themes-3)
</Box>
...
- Multi-part components now don’t export named parts (ie. `DialogRoot`, `DialogTrigger`, …)
  - Use dot notation for all multi-part components instead (ie. `Dialog.Root`, `Dialog.Trigger`, …) after importing like this: `import { Dialog } from "@radix-ui/themes"`.
- Component prop definitions and internal helpers are no longer available from the root `@radix-ui/themes` import entry point.
  - Import them from `@radix-ui/themes/props` and `@radix-ui/themes/helpers` instead. - Note: you might need to use `"moduleResolution": "NodeNext"` with your compiler to access these paths. - Note: changes to prop defs and helpers won’t be covered by semver.
    Props

Props

- The `width` and `height` props on layout components don't map to space scale anymore
  - Find and replace your `width` and `height` prop usage with the corresponding [space scale](https://github.com/radix-ui/themes/blob/main/packages/radix-ui-themes/src/styles/tokens/space.css) steps, e.g. `width="9"` would need to be changed to `width="64px"` or `width="var(--space-9)"`, and so on for other steps.
- The `shrink` and `grow` props on layout components were renamed
  - Use `flexShrink` and `flexGrow` instead

Colors

- `--color-surface-accent` was renamed to `--accent-surface`
  - If you were using this token for your custom components, make sure to replace the corresponding references
- `--accent-9-contrast`, `--red-9-contrast`, `--pink-9-contrast`, `--blue-9-contrast`, etc. were renamed to `--accent-contrast`, `--red-contrast`, `--pink-contrast`, `--blue-contrast` and so on.
  - If you were using these tokens for your custom components, make sure to replace the corresponding references
- `--color-focus-root`, `--color-selection-root`, `--color-autofill-root` were replaced with a focus color scale, e.g. `--focus-1` – `--focus-12`, and `--focus-a1` – `--focus-a12`.
  - If you were overriding the above tokens or using them in your custom components, you’ll need to references the new color scale.
