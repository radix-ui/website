...
Radix Themes provides a comprehensive array of components to make type arrangement in your apps simple and intuitive. Of these, the [Heading](/themes/docs/components/heading) and [Text](/themes/docs/components/text) components are the building blocks you will most frequently reach for. They are optimized for titles and body copy respectively and share common `size` and `weight` props for mapping with the type system.
