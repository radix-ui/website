...
Refined typography is integral for a well designed user interface and Radix Themes provides a comprehensive array of components to make type arrangement in your apps simple and intuitive.
...
The [Heading](/themes/docs/components/heading) and [Text](/themes/docs/components/text) components are the most common tools for rendering type. They are optimized for titles and body copy respectively.

```jsx live=true
<Heading mb="2" size="5">Typographic principles</Heading>
...
Compose formatting components to add [empasis](/themes/docs/components/em), signal [importance](/themes/docs/components/strong), highlight [code](/themes/docs/components/code) and more.
...
The typographic system is based on a 9 step scale, every step has a corresponding `font-size`, `line-height` and `letter-spacing` value which are designed to be used in combination.
...
You will be typically utilise this scale via the `size` prop on typographic components such as [Text](/themes/docs/components/text) and [Heading](/themes/docs/components/heading), though you can also access the values directly via [Tokens](/themes/docs/theme/token-reference) if needed.

## Font weight

The following weights are supported. Weights can be [customized](http://localhost:3000/themes/docs/theme/typography#weight-mapping) to map to your own typeface.
