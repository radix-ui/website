...
<Theme radius="small" applyBackgroundColor={false} style={{ maxWidth: 450 }}>
...
The concept of visual weight is well understood in UI design, and refers to the importance of an element in relation to other elements on the screen. For example, a primary action should be more dominant than a secondary action.

Rather than providing semantically named "primary" and "secondary" actions, Radix Themes instead exposes a set of distinct variants named according to their visual attributes, allowing you to define your own hierarchy of relative importance depending on your visual style.

Here is an example using [Button](/themes/docs/components/button).
...
In this example, `solid` is the most dominant, followed by `soft` and `ghost`.

By composing variants in combination with radius and other [theme settings](/themes/docs/components/theme) you can quickly create visual hierarchies and unique looking interfaces.
