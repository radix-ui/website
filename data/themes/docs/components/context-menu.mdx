...
This component inherits the props from the [Radix ContextMenu](/primitives/docs/components/context-menu) primitive.
...
Use the `color` prop to assign a specific color value, ignoring the global theme. You can also pass `color` to a specific item to override for semantic reasons (ie. destruction action).
