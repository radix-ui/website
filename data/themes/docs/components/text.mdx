...
This component is based on the `p` element and supports [common margin props](/themes/docs/theme/layout#margin-props).
...
Use the `as` prop to render text as a `p`, `div` or `span`. This prop is purely semantic and does not change the visual appearance.
...
Use the `highContrast` prop to add contrast within colored text.
