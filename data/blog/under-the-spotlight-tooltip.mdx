...
A Tooltip component is a brief and helpful message associated with an interactive element. It appears when the element receives keyboard focus or the mouse hovers over it.

It's possible to build a CSS only Tooltip. But chances are, it'll lack essential funcionalities.

There are many cases to consider. Such as accessibility concerns, control of delay and collision-aware positioning. Unfortunately, because of this complexity, custom-built Tooltip components aren't up to the task.
...
Our goal is to ensure the Tooltip component adheres to the [WAI-ARIA design pattern](https://www.w3.org/TR/wai-aria-practices-1.2/#tooltip) while providing the best possible developer and user experiences.
...
The Radix Accordion offers common configuration options via props. For example, you can configure it to allow multiple panels to be opened at the same time.
