...
## Put a subtitle here fam.

Primitives follow the [WAI-ARIA authoring practices](https://www.w3.org/TR/wai-aria-practices-1.2/) guidelines and are tested in a wide selection of modern browsers and commonly used assistive technologies.

What this means in practice is that we take care of implementation details like `aria` and `role` attributes and ensure basic functionality of core components. It means that users should be able to use our components as-is in many contexts and expect certain functionality like keyboard navigation to follow expectations based on these attributes.
...
In many Radix Primitives, we move focus based on the interactions a user normally takes in a given component. For example, in [`AlertDialog`](/primitives/docs/components/alert-dialog), when the modal is opened, focus is programatically moved to a `Cancel` button element to anticipate a response to the prompt.
