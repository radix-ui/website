...
Use the `delayDuration` and `skipDelayDuration` to control the timings at which a tooltip is shown. You can define these per-instance, but it is best to set default prop values for it so all your tooltips use the same timings.

These will always show instantly:
...
export default () => (
  <>
    ...
    <Tooltip.Root delayDuration={0}>
      <Tooltip.Trigger style={{ marginLeft: 10 }}>
        <ZoomOutIcon />
      </Tooltip.Trigger>
      <StyledContent>
        Zoom Out
