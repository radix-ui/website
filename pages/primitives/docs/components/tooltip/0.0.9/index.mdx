...
Because a disabled trigger will not fire events, adding a tooltip on a disabled trigger require a little bit more work. You will need to render the trigger as a wrapper (`span` for example) and add your disabled button inside. Furthermore, the disabled button will need a few more styles.
