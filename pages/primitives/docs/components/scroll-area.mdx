...
features:
  - Combines native scroll functionality with customizable visual parts
  - No weird underlying position movements via CSS transformations; all scrolling is native
  - Only shims pointer behaviors when interacting with the controls, meaning that keyboard controls are unaffected and work as users expect
