...
If you want to animate the content, chances are you want your animations to be relative to the correct origin. This is difficult because it depends on all the positioning props such as `side`, `sideOffset`, `align`, `alignOffset`, but also the size of the arrow if there is one, and can also change based on collisions.

For this, we expose a CSS custom property called `--radix-dropdown-menu-content-transform-origin` which you can use this way:
...
export default () => (
  <DropdownMenu.Root>
    ...
    <StyledContent>
      <StyledItem>Undo</StyledItem>
      <StyledItem disabled>Redo</StyledItem>
      <StyledItem>Cut</StyledItem>
      <StyledItem disabled>Copy</StyledItem>
      <StyledItem>Paste</StyledItem>
      <StyledArrow />
    </StyledContent>
  </DropdownMenu.Root>
...
export default () => (
  <DropdownMenu.Root>
    ...
    <StyledContent>
      ...
      <StyledCheckboxItem checked onCheckedChange={alert}>
        <StyledItemIndicator>
          <CheckIcon />
        </StyledItemIndicator>
        Show hidden files
